...
/**
 * CONTROLLER: Admin Users Management (BLUEPRINT SYNC)
 * SOP: Penanganan otoritas dan Pasal 378.
 */
...
export function useAdminUsersController() {
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    ...
    try {
      ...
      toast({ title: "Otoritas Diperbarui" });
  ...
  const handleResolveDebt = async (debtId: string, userId: string) => {
    ...
    toast({ title: "Lunas", description: "Blokir dibuka." });
