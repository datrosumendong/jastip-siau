...
import { ArrowLeft, Loader2, AlertCircle, Phone, Package, Truck, User, Trash2, ShieldAlert, Crown, Ban, Unlock } from 'lucide-react';
...
export default function AdminComplaintDetailView() {
  const { viewData, setView, goBack } = useView();
            ...
            <CardContent className="p-3 space-y-3 overflow-y-auto custom-scrollbar flex-1">
              {complaint.type === 'payment_issue' && (
                <div className="p-3 bg-red-600 rounded-xl text-white space-y-3 shadow-lg">
                   <p className="text-[9px] font-bold leading-relaxed uppercase">SOP: Blokir akses jika member menolak membayar tagihan resmi.</p>
              ...
              <div className="space-y-1">
                <span className="text-[7px] font-black text-muted-foreground uppercase">Update Status:</span>
                <Select defaultValue={complaint.status} onValueChange={handleUpdateStatus} disabled={updating}>
                  <SelectTrigger className="h-8 text-[8px] font-black uppercase rounded-xl"><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="open">Kasus Baru</SelectItem><SelectItem value="investigating">Investigasi</SelectItem><SelectItem value="resolved">Tuntas</SelectItem><SelectItem value="closed">Tutup</SelectItem></SelectContent>
                </Select>
              </div>
            </CardContent>
          </Card>
