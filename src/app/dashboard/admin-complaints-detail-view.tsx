 ...
 * SOP: Penanganan sengketa dan eskalasi Owner.
 * FIX: Menambahkan import User dan Package untuk membasmi ReferenceError.
 */
...
import { ArrowLeft, Loader2, AlertCircle, Phone, Ban, Unlock, ShieldAlert, Crown, User, Package } from 'lucide-react';
...
export default function AdminComplaintDetailView() {
  ...
  if (!complaint) return <div className="p-10 text-center uppercase font-black text-[10px] opacity-30">Tiket Tidak Ditemukan</div>;
    ...
    <div className="flex flex-col h-full space-y-3 overflow-hidden px-1">
      <div className="flex items-center gap-2 bg-white p-2 rounded-xl border border-primary/10 shadow-sm shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[7px] text-muted-foreground uppercase font-bold truncate">Ref: {complaint.orderId?.slice(-8)}</p>
        </div>
        {complaint.isEscalated && <Badge className="bg-amber-500 text-[6px] font-black uppercase">Eskalasi Owner</Badge>}
      </div>
        ...
        <div className="lg:col-span-1 h-full overflow-hidden flex flex-col">
          <Card className="border-none shadow-md bg-white rounded-2xl overflow-hidden flex flex-col h-full">
            <CardHeader className="bg-destructive/5 p-3 border-b shrink-0">
               <CardTitle className="text-[9px] font-black uppercase text-destructive flex items-center gap-2"><AlertCircle className="h-3.5 w-3.5" /> Masalah</CardTitle>
            </CardHeader>
            <CardContent className="p-3 space-y-3 overflow-y-auto custom-scrollbar flex-1">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] font-black uppercase text-primary p-2 bg-muted/20 rounded-lg">{complaint.userName}</p>
              </div>
              <div className="space-y-1">
                <span className="text-[7px] font-black text-muted-foreground uppercase tracking-widest">Ganti Status:</span>
                <Select defaultValue={complaint.status} onValueChange={handleUpdateStatus} disabled={updating}>
                  ...
                  <SelectContent>
                    <SelectItem value="open" className="text-[8px] font-black uppercase">Antrean</SelectItem>
                    ...
                    <SelectItem value="closed" className="text-[8px] font-black uppercase">Tutup</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="pt-2 border-t border-dashed">
                 <Button variant="outline" className="w-full h-8 text-[8px] font-black uppercase rounded-lg border-green-200 text-green-600" onClick={() => openWhatsAppChat(complaint.userWhatsapp, "Halo, saya Admin Jastip Siau terkait laporan Anda.")}>Hubungi Pelapor</Button>
              </div>
            </CardContent>
          </Card>
        </div>
